...
def generate_fasta(transcript_file):
    ...
    if not more_than_one:
        return False
    with open(path.join('/opt/current_project/results/trial_seqs/'+\
                        human_transcript_id_orig + source), 'w') as f:
        f.write(current_fasta)

if path.exists('/opt/current_project/results/trial_seqs/'):
    shutil.rmtree('/opt/current_project/results/trial_seqs/')
os.makedirs('/opt/current_project/results/trial_seqs/')
